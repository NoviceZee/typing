...
export class AppErrorBoundary extends React.Component<React.PropsWithChildren, State> {
  ...
  render() {
    ...
    return <main className="grid min-h-screen place-items-center px-5 text-paper"><section className="max-w-lg rounded-lg border border-paper/10 bg-ink-900 p-8 text-center"><p className="font-mono text-xs uppercase tracking-[0.2em] text-brass">Something slipped</p><h1 className="mt-4 text-2xl font-semibold">This page could not finish loading.</h1><p className="mt-3 text-sm leading-6 text-paper/50">Your saved practice data is untouched. Refresh the page to try again.</p><button className="landing-button-primary mt-7" onClick={() => location.reload()}>Refresh page</button></section></main>;
  }
}
