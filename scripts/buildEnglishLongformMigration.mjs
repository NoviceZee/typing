...
const migration = renderMigration(allArticles);
...
function validateMigration(sql) {
  if (/^\+/m.test(sql)) throw new Error("Generated SQL contains a patch-style leading plus sign.");
  const createIndex = sql.indexOf("create temporary table");
  const updateIndex = sql.indexOf("update public.passages");
  const insertIndex = sql.indexOf("insert into public.passages");
  const validationIndex = sql.indexOf("do $$");
  if (!(createIndex >= 0 && createIndex < updateIndex && updateIndex < insertIndex && insertIndex < validationIndex)) {
    throw new Error("Generated SQL statements are out of order.");
  }
  if (!sql.trimEnd().endsWith("end $$;")) throw new Error("Generated SQL validation block must be last.");
}
